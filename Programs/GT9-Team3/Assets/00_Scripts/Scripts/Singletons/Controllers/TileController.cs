...
public class TileController : MonoBehaviour
{
    ...
    public void ReceiveStageData(StageData stageData)
    {
        if (stageData != null)
        {
            ...
            //Debug.Log("roundLevel" + roundLevel);
            if (!isHardMode)
            {
    ...
    /// 월드 변경시 타일 변경
    /// </summary>
    /// <param name="level">worldLevel</param>
    ...
    private void SetTileGrid(int tileLength)
    {
        ...
        for (int row = 0; row < tileLength; row++)
        {
            for (int col = 0; col < tileLength; col++)
            {
                ...
                TileModel selectedTile = tileModels.FirstOrDefault(
                    tile => tile.tileCategory == TileCategory.Grid
                    );
            ...
            for (int row = 0; row < 3; row++)
            {
                for (int col = 0; col < 3; col++)
                {
                    ...
                    else
                    {
                        ...
                        TileModel selectedTile = tileModels.FirstOrDefault(
                            tile => tile.tileCategory == TileCategory.Castle
                            );
    ...
    private TileShape GetWeightedShape()
    {
        var shapeWeights = new Dictionary<TileShape, float>
        {
            { TileShape.Straight, 0.2f },
            { TileShape.Corner, 0.3f },
            { TileShape.TShape, 0.3f },
            { TileShape.Cross, 0.2f }
        };

        float total = shapeWeights.Values.Sum();
        float rand = Random.Range(0f, total);
        float cumulative = 0f;

        foreach (var kvp in shapeWeights)
        {
        ...
        return TileShape.TShape;
    }

    private int GetWeightedTowerSlotCount()
    {
        var slotWeights = new Dictionary<int, float>
        {
            { 0, 0f },
            { 1, 0.3f },
            { 2, 0.4f },
        ...
        float total = slotWeights.Values.Sum();
        float rand = Random.Range(0f, total);
        float cumulative = 0f;

        foreach (var kvp in slotWeights)
        {
            cumulative += kvp.Value;
            if (rand <= cumulative)
                return kvp.Key;
    ...
    {
        var normalTiles = tileModels
                            .Where(tile => tile.tileCategory == TileCategory.Normal)
                            .ToList();

        TileShape targetShape = GetWeightedShape();
        int targetSlotCount = GetWeightedTowerSlotCount();

        var filteredTiles = normalTiles
            .Where(tile => tile.tileShape == targetShape && tile.towerSlotCount == targetSlotCount)
            .ToList();

        if (filteredTiles.Count == 0)
        {
            Debug.LogWarning("조건에 맞는 타일이 없습니다.");
            return null;
        }

        TileModel selectedTile = filteredTiles[Random.Range(0, filteredTiles.Count)];

        return selectedTile.tilePrefab;
    }

    private GameObject GetRoadTile_BossMode()
    {
        var normalTiles = tileModels
                            .Where(tile => tile.tileCategory == TileCategory.Normal &&
                            tile.towerSlotCount == 3 ||
                            tile.towerSlotCount == 4)
                            .ToList();

        TileShape targetShape = GetWeightedShape();

        var filteredTiles = normalTiles
            .Where(tile => tile.tileShape == targetShape)
            .ToList();
    ...
    {
        GameManager.Instance.DestroyOfType<TileSpawner>();
    }
    ...
    private void CreateSpawner()
    {
        if (spawnTransform != null)
        {
            ...
            TileModel selectedTile = tileModels.FirstOrDefault(
                tile => tile.tileCategory == TileCategory.Spawner
                );
